      ...
      if (err) {
        ...
        for (i = 0; i < cts.length; i++) {
          try {
            ...
            if (content.fields.city.toString() !== obj.fields.city.toString()) {
              console.log("Different city : " + content.fields.name.toString())
              continue;
            }
            if (istest && !content.fields.isdevacc) {
              console.log("Couldn't send a test request to : " + content.fields.name.toString())
              continue;
            }
            console.log("Sending request to : " + content.fields.name.toString());
            var fields = {};
            fields.name = {
              fa: obj.fields.name,
              en: obj.fields.name
...
config.webhooks.submitstartuprequest = submitstartuprequest;
exports.submitstartuprequest = submitstartuprequest;