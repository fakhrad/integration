...
function submitinvestmentrequest() {
  ...
  var submittopartners = function (
    ...
    console.log("start loading rules :" + JSON.stringify({
      contentType: "5e8886b6f0356f001334bb88",
      status: "published",
      "sys.spaceId": obj.sys.spaceId,
      "fields.investmenttype": obj.fields.options
    }));
    Contents.find({
      contentType: "5e8886b6f0356f001334bb88",
      status: "published",
      ...
      if (err) {
        ...
        for (i = 0; i < rules.length; i++) {
          ...
          var matched = false;
          if (rule.fields.investmenttype) {
            for (j = 0; j < rule.fields.investmenttype.length > 0; j++) {
              var type = rule.fields.investmenttype[j].toString();
              if (obj.fields.options.indexOf(type) != -1)
                matched = true;
            }
          }
          if (!matched)
...
config.webhooks.submitinvestmentrequest = submitinvestmentrequest;
exports.submitinvestmentrequest = submitinvestmentrequest;